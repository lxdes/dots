...
selscroll(int orig, int n)
{
	...
	if (BETWEEN(sel.nb.y, orig, term.bot) != BETWEEN(sel.ne.y, orig, term.bot)) {
		...
		sel.ob.y += n;
		sel.oe.y += n;
		miny = IS_SET(MODE_ALTSCREEN) ? term.top : term.row - TSCREEN.size;
		maxy = IS_SET(MODE_ALTSCREEN) ? term.bot : term.row - 1 + TSCREEN.off;
		if (sel.ob.y < miny || sel.ob.y > maxy ||
		    sel.oe.y < miny || sel.oe.y > maxy) {
			selclear();
		} else {
			selnormalize();
		}
...
externalpipe(const Arg *arg)
{
	...
	newline = 0;
	for (n = term.row - term.screen[0].size; n < term.row; n++) {
		line = term.screen[0].buffer[
			(n + term.screen[0].cur + term.screen[0].size) %
			term.screen[0].size];
		if (!line)
			continue;
		for (lastpos = term.col; lastpos > 0 && line[lastpos - 1].u == ' ';
				lastpos--)
			;
		for (i = 0; i < lastpos; i++)
			if (xwrite(to[1], buf, utf8encode(line[i].u, buf)) < 0)
				break;
		if (lastpos > 0 &&
				(newline = line[lastpos - 1].mode & ATTR_WRAP))
			continue;
		if (xwrite(to[1], "\n", 1) < 0)
			break;
		newline = 0;
